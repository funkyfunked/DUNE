...
fun HomeToolbar(
    ...
    Toolbar {
        Row(horizontalArrangement = Arrangement.spacedBy(23.dp)) {
            ...
            // Live TV Icon + Label
            IconButton(onClick = openLiveTv, modifier = Modifier.width(105.dp).height(37.dp), colors = IconButtonDefaults.colors(containerColor = Color.Transparent)) {
                Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                    Icon(
                    ...
                    Spacer(Modifier.width(8.dp))
                    org.jellyfin.androidtv.ui.base.Text(
                        text = stringResource(R.string.lbl_live_tv)
                    )
                }
            }
            // Settings Icon + Label
